...
import { RunInsert } from './types';

/**
 * Create a run log entry for a job execution
 */
 ...
 * Using approximate pricing for Gemini models
 */
export function calculateCostEstimate(
  ...
  const pricing: Record<string, { input: number; output: number }> = {
    'gemini-2.0-flash-exp': { input: 0.075, output: 0.30 }, // Free tier during preview
...
/**
 * Helper to log a worker execution
 */
